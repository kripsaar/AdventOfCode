class Path:
    def __init__(self, connections: dict, visited: list = ["start"], current_node: str = "start", small_repeated: bool = False):
        self.connections = connections
        self.visited = visited
        self.current_node = current_node
        self.small_repeated = small_repeated
    ...
    def step(self):
        ...
        for candidate in candidates:
            small_repeated = self.small_repeated
            if candidate.islower() and candidate in self.visited:
                if self.small_repeated or candidate == "start":
                    continue
                small_repeated = True
            new_visited = self.visited.copy()
            new_visited.append(candidate)
            paths += Path(self.connections, new_visited, candidate, small_repeated).step()
        return paths
