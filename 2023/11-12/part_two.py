...
class Galaxy:
    ...
    def move_x(self, offset):
        self.pos = (self.pos[0] + offset - 1, self.pos[1])
...
def parse_input(filename: str, expansion_multiplier: int):
    universe = []
    galaxy_x_map = {}
    non_empty_x = set()
    max_x = 0
    ...
    for x, galaxies in galaxy_x_map.items():
        ...
        for smaller_x in smaller_empty_x:
            for galaxy in galaxies:
                galaxy.move_x(expansion_multiplier)
    return universe

universe = parse_input('input-11', 1_000_000)
sum = 0
