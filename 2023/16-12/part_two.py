...
def follow_ray(layout: dict[tuple[int, int], str], pos = (0, 0), direction: Direction = Direction.RIGHT, visited: dict[tuple[int, int], set[Direction]] = None):
    if not visited:
        visited = {}
    while True:
...
for x in range(x_length):
    max_visited_count = max(max_visited_count, len(follow_ray(layout, (x, 0), Direction.DOWN)))
    max_visited_count = max(max_visited_count, len(follow_ray(layout, (x, y_length - 1), Direction.UP)))
for y in range(y_length):
    max_visited_count = max(max_visited_count, len(follow_ray(layout, (0, y), Direction.RIGHT)))
    max_visited_count = max(max_visited_count, len(follow_ray(layout, (x_length - 1, y), Direction.LEFT)))
